...
def get_md5_from_series(input_iterable: Iterable) -> str:
    ...
    :param input_iterable: Typically a Pandas ``DataFrame`` row, but can be any Pandas ``Series``.

    It returns MD5 hash created from the input values.
    """
...
def get_md5_series_from_dataframe(input_dataframe: pd.DataFrame,
    ...
    :param input_dataframe: Pandas ``DataFrame`` to be create MD5 hashes for.
    :param columns: If only wanting to use specific columns to calculate the hash, specify these here.

    It returns MD5 hashes, one for every row in the input Pandas ``DataFrame``.
    """
...
def add_md5_hash_column(input_dataframe: pd.DataFrame, md5_column_name: str = 'md5_hash',
    ...
    :param input_dataframe: Pandas ``DataFrame`` to be create MD5 hashes for.
    :param md5_column_name: Name for the new column containing the MD5 hashes.
    :param columns: If only wanting to use specific columns to calculate the hash, specify these here.

    It returns Copy of the input_dataframe with a new column containing the MD5 hash for every row.
    """
