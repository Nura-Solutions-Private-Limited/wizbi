...
from db.dbutils.hashing import add_md5_hash_column
...
class EtlLoad:
    ...
    def load_dim(self, load_id, data):
        '''
            ...
            for table in tables:
                ...
                if table_type == 'dim':
                    ...
                    for source_df in all_source_df:
                        ...
                        if first_load:
                            logger.info(f'First data load, loading {source_df[source_columns].shape[0]} rows in table - {target_table}') # NOQA
    ...
    def load_fact(self, load_id, last_load_id, last_load_date, data):
        '''
        Load fact table
        '''
        dataMapping = DataMapping()
            ...
            for table in tables:
                ...
                if table_type == 'fact':
                    for column in columns:
                        ...
                        if column.get('LookupColumn'):  # 'creditLimit':
                            # get ETLKeys values
    ...
    def load_data(self, job_id):
        ...
        _ = self.load_dim(load_id=load_id, data=data)
        ...
        try:
            # Re-Loop through etl json and load dimension table fks
            for schema in data:
                # Get table data
                tables = schema.get('Tables')
                for table in tables:
            ...
            # Update job status to completed with errors
            current_time = datetime.now()
            ...
            logger.error("Error in etl load :{}".format(str(ex)))
            raise ex
