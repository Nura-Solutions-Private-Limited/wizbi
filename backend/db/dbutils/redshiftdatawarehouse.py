...
class RedshiftDatawarehouse:
    ...
    def createTimeid(self, dW, table):
       ...
       self.connection.execute('ALTER TABLE "{}" ADD COLUMN "TimeID" BIGINT NOT NULL default 1;'.format (table))
       self.connection.execute('ALTER TABLE "{}"ADD COLUMN load_id BIGINT NOT NULL default 1;'.format (table))
    ...
    def createTable(self, dW, table, columnName, columnType, Pkey, Fkey):
        ...
        if table.lower() in existing_tables:
            ...
            if columnName.lower() not in (existing_columns):
                ...
                """ if Pkey:
                    ...
                    # Alter column to set data type to integer with auto-increment
                    self.connection.execute('ALTER TABLE "{}" ALTER COLUMN "{}" inetger;'.format(table, columnName))
 """
                if Fkey:
                    for key in Fkey:
                        refTable = key.split('.')[0]
                        if refTable.lower() not in existing_tables:
                            # mycursor.execute('CREATE TABLE '+dW+'.'+refTable+'(ID INT,PRIMARY KEY (ID));')
                            self.connection.execute('CREATE TABLE "{}" ("RBID" INTEGER PRIMARY KEY IDENTITY(1,1));'.format(refTable))


                        refColumn = key.removeprefix(refTable + '.')
                        foreignKey = refTable + '_' + refColumn
                        if foreignKey.lower() not in (existing_columns):
            ...
            self.connection.execute('CREATE TABLE "{}" ("RBID" INTEGER PRIMARY KEY IDENTITY(1,1) ,"{}" {});'.format(table, columnName, columnType))
            if Fkey:
                for key in Fkey:
                    refTable = key.split('.')[0]
                    if refTable.lower() not in existing_tables:
    ...
    def create_sqlquery_for_reports(self):
        ...
        for tName in schema_dict['Tables']:
            if tName.get('type') == 'dim':
                ...
                if count > 0:
                    attr_dict[str( tName["TargetTable"]).lower()].extend(str_list)
                else:
                    attr_dict[str(tName["TargetTable"]).lower()].extend(nonstr_list)
            ...
            if tName.get('type') == 'fact':
                for cName in tName['Columns']:
                    for keys in cName['ETLKeys']:
                        ...
                        # Define the replacement values
                        attrs = ','.join(str(attr_dict[keys.get("LookupDim")]).lower())
                        metric = cName["TargetColumn"].lower()
                        lookupDim = keys.get("LookupDim").lower()
                        # Daily report
...
if __name__ == '__main__':
    datawarehouse =RedshiftDatawarehouse('test')
