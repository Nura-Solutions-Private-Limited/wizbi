...
class FileDataUpload:
    ...
    def get_file_mapping(self, file_name, all_file_mapping):
        '''
        return file mapping if exist else return nothing
        '''
        for file_mapping in all_file_mapping:
            if file_name == file_mapping.get("file_name"):
                return file_mapping
        return None

    def get_file_preview(self):
        '''
        Read file to fetch column names, infer column data types by
        reading 1000 rows and return 10 rows sample data for each column

        read source data mapping for the pipeline, if there is any then read
        that include only those column and data type in file preview
        '''
        # get saved column from database
        source_db_mapping = self.db.query(Source_Db_Mapping).\
            filter(Source_Db_Mapping.pipeline_id == self.pipeline_id).first()

        all_file_mapping = None
        if source_db_mapping:
            all_files = source_db_mapping.user_input
            all_file_mapping = all_files.get('files')
        ...
        try:
            for file in self.file_list:
                ...
                # get the saved file column selection with data type and format and include those in response
                saved_format = {}
                if all_file_mapping:
                    file_mapping = self.get_file_mapping(file_name=file_name, all_file_mapping=all_file_mapping)

                    saved_columns = []
                    for file_column in file_mapping.get('file_columns'):
                        print(file_column.get('column_name'), file_column.get('data_type'), file_column.get('format'))
                        saved_columns.append(file_column.get('column_name'))
                        saved_format[file_column.get('column_name')] = {'data_type': file_column.get('data_type'),
                                                                        'format': file_column.get('format')}

                    # keep only those column which has been selected by user in previous attempt
                    df = df[saved_columns]

                # Get all column names and data types in dictionary
                df_data_types = df.dtypes.astype(str).to_dict()
                df = df.fillna('')

                # replace object data type with string
                for key, value in df_data_types.items():
                    # print(key, value)
                    if value == 'object':
                        df_data_types.update({key: 'string'})

                file_columns.append({"file_name": file_name,
                                     "file_columns": df_data_types,
                                     "file_columns_data": df.head(10).to_dict('list'),
                                     'saved_column_datatype': saved_format})
            return file_columns
    ...
    def drop_columns(self, df, columns):
        ...
        '''
        columns_to_be_dropped = list(set(df.columns.str.lower().to_list()) - set(columns))
    ...
    def convert_data_types(self, df, data_types):
        ...
        for col, dtype in data_types.items():
            try:
                if dtype.get('data_type').lower() == 'datetime':
                    df[col.lower()] = pd.to_datetime(df[col.lower()], format=dtype.get('format'))
                else:
                    df[col.lower()] = df[col.lower()].astype(dtype.get('data_type'))
            except (ValueError, TypeError):
                logger.error(f"Error converting column {col} to type {dtype.get('data_type')}.")
                raise ValueError(f"Error converting column {col} to type {dtype.get('data_type')}.")
        return df
        ...
        for files in file_data_mappings:
            if files.get('file_name') == file_name:
                ...
                for file_column in file_columns:
                    ...
                    column_data_format = file_column["format"]

                    data_types[column_name] = {"data_type": column_data_type, "format": column_data_format}

                # drop column which is not in user selected input
                df = self.drop_columns(df, column_names)
