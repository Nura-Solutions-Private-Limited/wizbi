"""wizbi database in postgresql

Revision ID: 675a20d984a7
Revises: 
Create Date: 2025-02-03 10:32:00.772504
...
def upgrade() -> None:
    ...
    op.execute(sa.DDL("""
    alter table `permissions` add constraint fk_permissions_role_id foreign key (role_id) references role(id)
    """))

    op.execute(sa.DDL("""
    create table if not exists `db_conn` (`id` int not null auto_increment,
    ...
    op.execute(sa.DDL("""
    alter table db_conn add constraint fk_conn_user foreign key (user_id) references user(id)
    """))
    ...
    op.execute(sa.DDL("""
    alter table connection_ext add constraint `fk_db_conn` foreign key (db_conn_id) references `db_conn` (`id`)
    """))

    op.execute(sa.DDL("""
    create table if not exists rest_api_conn(id int not null auto_increment,
                                            method varchar(10),
                                            url varchar(500),
                                            params json,
                                            authorization json,
    ...
    op.execute(sa.DDL("""
    alter table rest_api_conn add constraint fk_rest_api_conn_id foreign key (db_conn_id) references db_conn(id)
    """))

    op.execute(sa.DDL("""
    create table if not exists `db_er` (`id` int not null auto_increment,
                                        `user_id` int default null,
    ...
    op.execute(sa.DDL("""
    CREATE TABLE IF NOT EXISTS `pipeline` (`id` int NOT NULL AUTO_INCREMENT,
                                           `user_id` int DEFAULT NULL,
    ...
    op.execute(sa.DDL("""
    ALTER TABLE pipeline ADD CONSTRAINT `fk_db_conn_dest` FOREIGN KEY (db_conn_dest_id) REFERENCES `db_conn` (`id`)
    """))

    op.execute(sa.DDL("""
    ALTER TABLE pipeline ADD CONSTRAINT `fk_db_conn_source`
                      FOREIGN KEY (db_conn_source_id) REFERENCES `db_conn` (`id`)
    """))

    op.execute(sa.DDL("""
    ALTER TABLE pipeline add constraint fk_user foreign key (user_id) references `user`(id)
    """))
    ...
    op.execute(sa.DDL("""
    ALTER TABLE source_db_mapping add CONSTRAINT `fk_pipeline` FOREIGN KEY (pipeline_id) REFERENCES `pipeline` (`id`)
    """))
    ...
    op.execute(sa.DDL("""
    ALTER TABLE job add CONSTRAINT `fk_job_pipeline` FOREIGN KEY (pipeline_id) REFERENCES `pipeline` (`id`)
    """))
    ...
    op.execute(sa.DDL("""
    ALTER TABLE audit add CONSTRAINT `fk_audit_pipeline` FOREIGN KEY (pipeline_id) REFERENCES `pipeline` (`id`)
    """))

    op.execute(sa.DDL("""
    ALTER TABLE audit add CONSTRAINT `fk_audit_job` FOREIGN KEY (job_id) REFERENCES `job` (`id`)
    """))
    ...
    op.execute(sa.DDL("""
    ALTER TABLE report add CONSTRAINT `fk_report_pipeline` FOREIGN KEY (pipeline_id) REFERENCES `pipeline` (`id`)
    """))
    ...
    op.execute(sa.DDL("""
    ALTER TABLE pipeline_schedule add CONSTRAINT `fk_schedule_pipeline`
    ...
    op.execute(sa.DDL("""
    ALTER TABLE dashboard add  CONSTRAINT `fk_group_id` FOREIGN KEY (group_id) REFERENCES `user_group` (`id`)
    """))
    ...
    op.execute(sa.DDL("""
    ALTER table ga_metric add constraint fk_dimension_id foreign key (dimension_id) references ga_dimension(id)
    """))
